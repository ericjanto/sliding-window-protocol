...
def run_sim(test_results_file_name, window_size, delay_ms, rcv_window_size):
    # Simulate link layer via loopback interface
    os.system('sudo tc qdisc del dev lo root')
    os.system(
        f'sudo tc qdisc del dev lo root sudo tc qdisc add dev lo root netem loss 5% delay {delay_ms}ms rate 10mbit')

    n = 2
    sender_no = 4
    receiver_no = 4
    receive_file_name = 'rfile4.jpg'
    retry_timout = 25
...
def test_varied_window_sizes():
    test_fn = '_results_test_selectiveRepeat.txt'
...
def test_varied_delay_times():
    ...
    for d in delays:
        run_sim(test_fn, window_size, d, window_size)
